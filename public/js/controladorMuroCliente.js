...
inputFiltro.addEventListener('keyup', mostrarListaActividades);
mostrarListaLugares();
...
function getListarLugaresSeguidos() {
    ...
    if(usuarioInfo.lugares == undefined) {
        ...
        for (let i = 0; i < usuarioInfo.lugares.length; i++) {
            // buscar informacion del lugar por i
            lugaresInfoCompleta.push(buscar_lugar(usuarioInfo.lugares[i]));
        }
    }
    return lugaresInfoCompleta;
}
...
function mostrarListaActividades() {
    ...
    for (let i = 0; i < listaActividades.length; i++) {
        ...
        botonReservar.dataset.id_actividad = listaActividades[i]['_id']
        botonReservar.addEventListener('click', visualizarActividad);
